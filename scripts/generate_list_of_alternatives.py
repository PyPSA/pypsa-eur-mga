...
def get_mga_components_totals(n):
    ...
    for comp in ["generators", "storage_units"]:
        carriers = list(
            getattr(n, comp).loc[getattr(n, comp).p_nom_extendable].carrier.unique()
        )
...
def get_mga_component_groups(n):
    ...
    mga_groups["transmission"] = list(
        np.unique(np.concatenate((mga_groups["lines"], mga_groups["links"])))
    )
