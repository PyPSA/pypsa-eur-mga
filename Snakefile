...
checkpoint generate_list_of_alternatives:
    ...
    output: "results/alternatives/elec_s_{clusters}_l{ll}_{opts}_cat-{category}.txt"
    script: "scripts/generate_list_of_alternatives.py"

rule generate_alternative:
    input: "results/networks/elec_s_{clusters}_l{ll}_{opts}.nc"
    output: "results/networks/elec_s_{clusters}_l{ll}_{opts}_tol{epsilon}_cat-{category}_obj-{objective}.nc"
    log:
        solver="logs/elec_s_{clusters}_l{ll}_{opts}_tol{epsilon}_cat-{category}_obj-{objective}_solver.log",
        python="logs/elec_s_{clusters}_l{ll}_{opts}_tol{epsilon}_cat-{category}_obj-{objective}_python.log",
        memory="logs/elec_s_{clusters}_l{ll}_{opts}_tol{epsilon}_cat-{category}_obj-{objective}_memory.log"
            ...
            for opts in wildcards['opts']:
                for epsilon in wildcards['epsilon']:
                    for category in wildcards['category']:
                        ...
                        for obj in obj_list:              
                            input.append(
                                "results/networks/elec_s_{clusters}_l{ll}_{opts}_tol{epsilon}_cat-{category}_obj-{objective}.nc".format(
                                    clusters=cluster,
                                    ll=ll,
                                    opts=opts,
                                    epsilon=epsilon,
